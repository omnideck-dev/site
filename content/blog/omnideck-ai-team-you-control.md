...
<div class="callout info"><strong>TL;DR:</strong> omnideck is an open-source, local-first, multi-agent AI workbench with a real UI, not just a terminal. <a href="/install.html">Download omnideck</a> for macOS, Windows, or Linux, follow the guided setup, and bring a local Ollama model or a cloud API key. Full steps are in the <a href="https://www.omnideck.dev/docs/getting-started.html">getting started guide</a>.
</div>
...
The desktop app is now the default installation path:

1. **[Install omnideck](https://www.omnideck.dev/install.html).** Choose the native installer for your operating system and architecture.
2. **Select Set up omnideck.** The app prepares Podman and the local runtime, including any required operating-system permissions or restart.
3. **Open your workbench.** Connect an LLM provider or local Ollama and start creating agents.

That's it. Your data lives in `~/Omnideck` and survives updates.

Full instructions, real installation screenshots, and the advanced CLI path are in the [getting started docs](https://www.omnideck.dev/docs/getting-started.html). The [GitHub repo](https://github.com/omnideck-dev/omnideck) has the source, and it's Apache 2 licensed, so you can look under the hood or fork it outright.
